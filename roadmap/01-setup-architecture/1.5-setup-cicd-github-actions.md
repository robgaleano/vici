...
- Current status: executed — one manual step pending 🟢
...
> **Remaining manual step:** generate an `EXPO_TOKEN` on the Expo dashboard (Account → Settings → Access Tokens) and add it as a GitHub Actions secret named `EXPO_TOKEN` (repo Settings → Secrets and variables → Actions). `build.yml` is committed and waiting on this secret; once added, the manual "Run workflow" button will queue an EAS build. See the walkthrough at the end of this section.

### Execution outcome (2026-06-26)

- ✅ Phase A — Expo account (`robgaleano`), EAS CLI 20.4.0, project linked (`@robgaleano/vici`, id `3ab8c67a-…`)
- ✅ Phase B — `apps/mobile/eas.json` with 3 profiles; `preview` → `.apk` + iOS simulator `.app`
- ✅ Phase C — 3 `EXPO_PUBLIC_*` env vars in EAS `preview`; service-role key confirmed absent client-side
- ✅ Phase D — Android `.apk` and iOS-sim `.app` both built on EAS cloud; iOS launched in simulator
  - ⚠️ App crashes at launch on Intel-Mac simulator (`SIGSEGV` in `turbomodulemanager`, New-Arch/RN 0.83). **App-runtime bug, not a pipeline issue** — deferred to a feature epic.
- ✅ Phase E — `.github/workflows/ci.yml` (lint + typecheck) green on PR #3. Caught two real bugs: missing `@vici/supabase` tsconfig, and a pnpm double-version error.
- ✅ Phase F — `.github/workflows/build.yml` committed (`workflow_dispatch` active, `push:[main]` commented). Pending only the `EXPO_TOKEN` secret.

**Bonus fixes made during execution (not in original plan):**

- Added `apps/mobile` bundle identifiers (`com.robgaleano.vici`) so `--non-interactive` CI builds don't hang on a prompt.
- Created `packages/supabase/tsconfig.json` (was missing → `typecheck` failed with TS6053).
- Dropped the redundant `version:` input from `pnpm/action-setup` — newer versions error when the version is also in `package.json`'s `packageManager` (single source of truth).
- Added `.turbo/` to `.gitignore`.
- **Monorepo note:** EAS auto-detects the pnpm workspace via `pnpm-workspace.yaml` and uploads from the repo root, so `@vici/*` packages resolve in the cloud build. The `cli.version` floor in `eas.json` enforces a CLI new enough for this detection.
...
- [x] Create a free Expo account (if none) and verify email — `robgaleano`
- [x] Install the EAS CLI (`pnpm add -g eas-cli`) and `eas login` — CLI 20.4.0
- [x] From `apps/mobile/`, run `eas init` to link the project — id `3ab8c67a-38c9-46d9-bfe6-18ca19de9833`
- [x] (added) Set iOS/Android bundle identifiers `com.robgaleano.vici` so `--non-interactive` builds don't prompt

### Phase B — `eas.json` build profiles (D5)

- [x] Create `apps/mobile/eas.json` with `development`, `preview`, `production` profiles (scaffolded via `eas build:configure`)
- [x] Set `preview.android.buildType: "apk"` to produce an installable `.apk`
- [x] Set `preview.ios.simulator: true` for the free simulator build
- [x] Commit `eas.json` (commit `07e48a5`)

### Phase C — EAS environment variables (D6)

- [x] Create `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY` as EAS env vars (real values, `preview` environment, `plaintext`)
- [x] Create `EXPO_PUBLIC_MAPBOX_TOKEN` as a placeholder EAS env var
- [x] Confirm the service-role key is **not** present anywhere client-side (grep clean)

### Phase D — First manual cloud build (validates D5)

- [x] `eas build --profile preview --platform android` → green, `.apk` produced
- [x] `eas build --profile preview --platform ios` (simulator) → `.app` built, installed and launched in the iOS Simulator
- [~] Confirm both artifacts launch the app — iOS launches then crashes at runtime on Intel-Mac simulator (New-Arch/RN 0.83); deferred app-runtime bug, not a pipeline issue

### Phase E — `ci.yml` quality gate (D2–D4)

- [x] Create `.github/workflows/ci.yml` triggered on `pull_request` to `main`
- [x] Steps: checkout → pnpm (from `packageManager`) → Node 24 + `cache: 'pnpm'` → `pnpm install --frozen-lockfile` → `pnpm lint` → `pnpm typecheck`
- [x] Open a test PR and confirm the gate runs green — PR #3, green
- [ ] (Optional) mark the gate a required status check on `main`

### Phase F — `build.yml` manual build workflow (D7)

- [ ] Generate an `EXPO_TOKEN` (Expo dashboard → access tokens) and add it as a **GitHub Actions secret** — **manual step remaining**
- [x] Create `.github/workflows/build.yml` with `workflow_dispatch` active and `push: [main]` commented out
- [x] Step: use `expo/expo-github-action` with the token → `eas build --profile preview --platform all --non-interactive --no-wait`
- [ ] Trigger it from the GitHub Actions tab and confirm it queues an EAS build — pending the token

### Phase G — Wrap-up

- [x] Update epic 1.5 status → Completed; check the steps that are genuinely done
- [x] Tick "Basic CI pipeline running on every PR" in the epic Definition of Done
- [x] Record the deferred items (test job, store submit, iOS signing) in the epic
